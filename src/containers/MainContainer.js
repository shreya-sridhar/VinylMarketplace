...
class MainContainer extends React.Component {
  ...
  downloadUsers = () => {
    fetch("https://calm-ravine-36992.herokuapp.com/users")
      .then((res) => res.json())
      .then((data1) =>
        fetch("https://calm-ravine-36992.herokuapp.com/records")
          .then((res) => res.json())
          .then((data2) =>
            fetch("https://calm-ravine-36992.herokuapp.com/sell_records")
              .then((res) => res.json())
              .then((data3) =>
                fetch("https://calm-ravine-36992.herokuapp.com/order_records")
                  .then((res) => res.json())
                  .then((data4) =>
                    fetch("https://calm-ravine-36992.herokuapp.com/orders")
                      .then((res) => res.json())
                      .then((data5) => {
                        this.setState({
                          users: data1,
  ...
  addToCart = (rec) => {
    if (!localStorage.token) {
      ...
      let user_find = this.state.orders.filter(
        (order) =>
          order.user.id === this.state.user.id && order.status === "in_cart"
      );
  ...
  createOrderRecord = (rec) => {
    debugger;
    let currOrder = this.state.orders.filter(
      (order) =>
        order.user.id === this.state.user.id && order.status === "in_cart"
    )[0];
    debugger;
  ...
  renderLoginPage = () => (
    <Login handleLoginOrSignup={this.handleLogin} act="login" />
  );
  renderSignUpPage = () => (
    <Login handleLoginOrSignup={this.handleSignup} act="signup" />
  );
  renderMessages = () => (
    <Messages sendMessage={this.sendMessage} user={this.state.user} />
  );
  renderHistory = () => (
    <History user={this.state.user} order_records={this.state.order_records} />
  );

  changeCartStatus = () => {
    let order_find = this.state.orders.find(
      (order) =>
        order.user.id === this.state.user.id && order.status === "in_cart"
    ).id;
    // change to single element later

    fetch(`https://calm-ravine-36992.herokuapp.com/orders/${order_find}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  deleteRecord = (order_rec) => {
    fetch(
      `https://calm-ravine-36992.herokuapp.com/order_records/${order_rec.id}`,
      {
        method: "DELETE",
      }
    )
      ...
      <div>
        ...
        <Switch>
          ...
          <Route
            ...
            render={(routerProps) => {
              let user = this.state.users.find(
                (user) => user.id == routerProps.match.params.slug
              );
              return user ? (
                <UserHome
                  user={user}
          ...
          <Route
            ...
            render={(routerProps) => {
              let user = this.state.users.find(
                (u) => u.id == routerProps.match.params.slug
              );
              return user ? (
                <Cart
                  user={user}
